...
describe('TodolistPageComponent', () => {
  const mockTodos: TodoEntity[] = [
    { id: '1', title: 'Task A', completed: false, createdAt: 1000 },
    { id: '2', title: 'Task B', completed: true, createdAt: 2000 },
  ];
  ...
  function createComponent(overrides: Record<string, any> = {}) {
    ...
    const mockCreateTodoUseCase = {
      execute: vi.fn().mockReturnValue(
        of({ id: '3', title: 'Task C', completed: false, createdAt: 3000 })
      ),
      ...overrides['createTodoUseCase'],
    };
    const mockUpdateTodoUseCase = {
      execute: vi.fn().mockReturnValue(
        of({ id: '1', title: 'Task A', completed: true, createdAt: 1000 })
      ),
    ...
    const component = runInInjectionContext(injector, () => new TodolistPageComponent());
    return {
      component,
      mockGetTodosUseCase,
  ...
  describe('displayedColumns', () => {
    it('should have the correct columns defined', () => {
      const { component } = createComponent();
      expect(component.displayedColumns).toEqual(['no', 'title', 'status', 'action']);
    });
  });
  ...
  describe('addTodo', () => {
    it('should call CreateTodoUseCase with the title', () => {
      const { component, mockCreateTodoUseCase } = createComponent();
      component.addTodo('Task C');
      expect(mockCreateTodoUseCase.execute).toHaveBeenCalledWith('Task C');
    });

    it('should prepend the new todo to the list', () => {
      const { component } = createComponent();
      component.addTodo('Task C');
      const todos = component.todos();
      expect(todos[0]).toEqual({ id: '3', title: 'Task C', completed: false, createdAt: 3000 });
  ...
  describe('toggleTodo', () => {
    it('should call UpdateTodoUseCase with toggled completed status', () => {
      ...
      // Task '1' is completed: false, so toggle sends completed: true
    ...
    it('should toggle a completed todo to incomplete', () => {
      const updatedTodo: TodoEntity = { id: '2', title: 'Task B', completed: false, createdAt: 2000 };
      const { component, mockUpdateTodoUseCase } = createComponent({
        updateTodoUseCase: { execute: vi.fn().mockReturnValue(of(updatedTodo)) },
      });
      // Task '2' is completed: true, so toggle sends completed: false
      component.toggleTodo('2');
      expect(mockUpdateTodoUseCase.execute).toHaveBeenCalledWith({
        id: '2',
        data: { completed: false },
      });
      const todo = component.todos().find(t => t.id === '2');
  ...
  describe('deleteTodo', () => {
    it('should call DeleteTodoUseCase with the correct id', () => {
    ...
    it('should close the modal and reset todoToDelete after deletion', () => {
      const { component } = createComponent();
      component.confirmDelete('1');
      component.deleteTodo();
    ...
    it('should not call DeleteTodoUseCase if todoToDelete is null', () => {
      const { component, mockDeleteTodoUseCase } = createComponent();
      // Don't call confirmDelete first
      component.deleteTodo();
